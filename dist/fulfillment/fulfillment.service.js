    ...
    async manualResolve(lineItemId, dto, correlationId) {
        ...
        if (lineItem.syncJob) {
            await this.syncJobRepository.update(lineItem.syncJob.id, {
                status: dto.newStatus === order_line_item_entity_1.FulfillmentStatus.CONFIRMED ? vendor_sync_job_entity_1.SyncJobStatus.COMPLETED : vendor_sync_job_entity_1.SyncJobStatus.DEAD_LETTER,
            });
        }
