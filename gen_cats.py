import base64
content = b"""/**
 * app/(operational)/admin/categories/page.tsx — Category Management (§6).
 */
      ...
      <header className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-0.5 text-sm text-[var(--color-muted-foreground)]">
            {loading ? "Loading..." : categories.length + " categor" + (categories.length !== 1 ? "ies" : "y") + " \\u00B7 " + totalProducts + " total products \\u00B7 " + totalActive + " active \\u00B7 " + totalStock.toLocaleString() + " units in stock"}
          </p>
        </div>
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/50 p-4" onClick={() => setModalOpen(false)} role="dialog" aria-modal="true" aria-label="New category">
          <form onClick={(e) => e.stopPropagation()} onSubmit={handleCreate} className={cn("w-full max-w-sm rounded-xl border p-6 shadow-xl","border-[var(--color-border)] bg-[var(--color-card)]","space-y-4")}>
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-semibold text-[var(--color-foreground)]">New Category</h2>
            ...
            <div>
              <label htmlFor="cat-name" className="mb-1 block text-sm font-medium text-[var(--color-foreground)]">Category Name</label>
...
with open('C:\\Users\\raghava\\OneDrive\\Desktop\\marketplace-order-fulfillment\\cats.b64', 'w', encoding='utf-8') as f:
    f.write(encoded)
