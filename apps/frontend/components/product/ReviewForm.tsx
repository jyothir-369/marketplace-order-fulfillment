...
export function ReviewForm() {
  ...
  return (
    <div className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-card)] p-6 shadow-v2 space-y-4">
      ...
      <ComingSoon note="Review submission is not wired yet — no backend receptor for reviews. This form is a visual demo only." />
      ...
      <button disabled className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-[var(--color-primary)] text-white text-sm font-bold opacity-50 cursor-not-allowed"><Send className="h-4 w-4" /> Submit Review</button>
    </div>
  );
}
